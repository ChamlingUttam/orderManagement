...
export const crudStore = create((set) => ({
  ...
  createItem: async (data) => {
    ...
    try {
      ...
      toast.error("Can't create the item");
    } finally {
      set({ isCreating: false });
    }
  },
  ...
  updateItem: async (id, data) => {
    ...
    try {
      const res = await api.put(`/crud/edit/${id}`, data);
      set((state) => ({
        items: state.items.map((item) => item._id === id ? res.data : item) // ✅ fixed
      }));
      toast.success("Updated");
    } catch (error) {
      toast.error("Can't update the item");
    } finally {
      set({ isUpdating: false });
    }
  },
  ...
  deleteItem: async (id) => {
    ...
    try {
      await api.delete(`/crud/delete/${id}`);
      set((state) => ({
        items: state.items.filter((item) => item._id !== id) // ✅ fixed
      }));
      toast.success("Successfully deleted");
    } catch (error) {
